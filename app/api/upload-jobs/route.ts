...
export async function GET(req: Request) {
  try {
    ...
    if (!due) {
      // Listado normal: solo lectura, sin reclamar nada
      const jobs = await UploadJob.find({ userId }).sort({ scheduledAt: 1 });
      return NextResponse.json(jobs);
    }

    // Flujo "due": reclamamos cada job atómicamente para evitar
    // que dos invocaciones solapadas (polling cada 20s, concurrencia
    // serverless, etc.) procesen el mismo job por duplicado.
    const claimedJobs = [];
    let job;
    while (
