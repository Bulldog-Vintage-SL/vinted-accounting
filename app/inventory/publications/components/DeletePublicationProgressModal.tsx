...
interface Props<T> {
  open: boolean
  jobs: Job<'deletePublication', T>[]
  isBusy: boolean
  onClose: () => void
  title?: string
  onRetryJob?: (job: Job<'deletePublication', T>) => void
}
...
function DeletePublicationProgressModal<T>({ open, jobs, isBusy, onClose, title, onRetryJob }: Props<T>) {
  ...
  const getSummaryTitle = () => {
    if (blockClose) return title ?? 'Eliminando publicaciones...'
    if (allFailed) return 'No se pudo eliminar ninguna publicación'
    if (failedCount > 0) return 'Eliminación completada con errores'
    return '¡Publicaciones eliminadas!'
  }

  const getSummarySubtitle = () => {
    if (blockClose) return 'No cierres esta ventana mientras se eliminan tus publicaciones.'
    if (allFailed) return 'Revisa los errores de cada publicación o cierra esta ventana.'
    if (failedCount > 0) return `${completedCount} eliminada(s), ${failedCount} con error.`
...
function JobStatusBadge({ status }: { status: JobStatus }) {
  switch (status) {
    case 'completed':
      return <span className="flex items-center gap-1.5 text-green-600 text-sm font-medium"><CheckCircle2 size={18} /> Eliminado</span>
    case 'failed':
      return <span className="flex items-center gap-1.5 text-red-600 text-sm font-medium"><XCircle size={18} /> Error</span>
    case 'processing':
      return <span className="flex items-center gap-1.5 text-blue-600 text-sm font-medium"><Loader2 size={18} className="animate-spin" /> Eliminando...</span>
    case 'retrying':
...
export default DeletePublicationProgressModal