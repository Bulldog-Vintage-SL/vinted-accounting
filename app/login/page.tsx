...
export const metadata = getSEOTags({
  ...
  description: "Accede a tu cuenta de Reventa Libertad con email y contraseña.",
...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  ...
  const showEmailMagicLink = Boolean(process.env.RESEND_API_KEY);
  const errorMessage = params.error
    ? (errorMessages[params.error] ?? errorMessages.Default)
    : null;

  return (
    <main className="min-h-screen bg-base-200 flex flex-col items-center justify-center p-4 gap-6">
      ...
      <LoginForm
        ...
        showEmailMagicLink={showEmailMagicLink}
