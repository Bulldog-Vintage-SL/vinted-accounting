...
export async function reuploadVestiaireItem(
  ...
  try {
    ...
    const resDelete = await deleteVestiaireItem(itemExternalId, publicationId);
    ...
    const uploadResult = await uploadVestiaireItem(
      modifiedListing,
      accountId
    );
    ...
    return {
      ok: true,
      message: "Publicación resubida correctamente en Vestiaire",
      data: {
        listingId: listing.id,
        newTitle,
        newDescription,
